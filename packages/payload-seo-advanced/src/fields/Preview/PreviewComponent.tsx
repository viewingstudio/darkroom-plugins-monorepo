...
export const PreviewComponent: React.FC<PreviewProps> = (props) => {
  ...
  const disableSiteNamePath = titlePath?.replace(/\.title$/, '.disableSiteName') ?? 'meta.disableSiteName'
  ...
  // Compose the display title with suffix
  const suffix = hasGlobalSettings && siteName && !disableSiteName
    ? ` ${titleSeparator} ${siteName}`
    : ''
  const displayTitle = metaTitle ? `${metaTitle}${suffix}` : ''
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {displayTitle || (metaTitle as string)}
          </a>
        </h4>
        <p
          style={{
